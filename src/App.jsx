...
// A relative path like "/api/..." only resolves correctly when the page's
// own address is actually ripscoregolf.com - true for the website, but
// never true inside the Capacitor iOS app, which serves the bundled app
// from its own internal address instead. Every API call is prefixed with
// this so it reaches the real, correct destination in both places, rather
// than silently trying (and failing) to fetch from wherever the app
...
const GAMES = {
  ...
  matchplay: {
    ...
    defaults: { maxOver: "", maxPutts: "", mulliganSegment: "", mulliganChallenge: "", prize: "", netScoring: true },
    rules: [
      "Head-to-head match play for exactly 2 players - the classic, original form of golf scoring.",
      "Each hole is its own contest: whoever has the lower net score wins that hole. Equal net scores halve the hole - nobody wins it.",
      "The match is tracked hole by hole as holes won, not total strokes - a big blow-up hole only ever costs you that one hole, same as a narrow loss would.",
      "The match ends the moment a player is ahead by more holes than remain to be played - for example, 3 up with only 2 holes left ends the match \"3 and 2\", and the remaining holes aren't needed to decide the outcome. The group can still keep entering scores for fun if they'd like to play out the full round.",
      ...
      "Per-hole handicapping (net score per hole) is defaulted to On, but can be turned off in game scoring settings. Match play handicap strokes are based on the difference between the two players.",
...
/* ===================================================================
   SIDE GAMES — merged directly into App.jsx (originally SideGames.jsx)
   so there's only one file to manage. Nothing below is renamed - same
   exact names as the standalone file, just no longer exported since
   everything using them now lives in this same file too.
=================================================================== */
...
const remoteAdapter = ({ load, save, clear }) => ({
...
const DEFAULT_PARS = [4, 5, 3, 4, 4, 3, 5, 4, 4, 4, 3, 5, 4, 4, 3, 4, 5, 4];
...
const storageKey = (roundId) => `sidegames:v${SCHEMA_VERSION}:${roundId}`;
...
function computeMatchPlayResult(round, computed) {
                ...
                <>
                  ...
                  {!leaveGroupConfirming && !deleteGroupConfirming && (
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16 }}>
                      <button className="gsc-link" style={{ fontSize: 14, color: "#8a8a80" }} onClick={() => setLeaveGroupConfirming(true)}>
                        Leave group
                      </button>
                      {(() => {
                        const g = myGroups.find((g) => g.id === selectedGroupId);
                        ...
                        return (
                          <button className="gsc-link" style={{ fontSize: 14, color: "#8a8a80" }} onClick={() => setDeleteGroupConfirming(true)}>
                            Delete group
                          </button>
                        );
                      })()}
        ...
        <div className="gsc-body">
          <div className="gsc-card gsc-no-select">
            <div style={{ fontSize: 13, color: "#4b4b45", lineHeight: 1.6 }}>
              ...
              <p style={{ margin: "0 0 14px" }}>
                RipScore is the golf app for your regular game and your not-so-regular game - the same crew every Saturday, or a one-off round with people you just met on the first tee. No scorecard math, no "wait, what were we betting on again?" RipScore keeps score, tracks who's up, and settles the debate before you're off the 18th green.
              </p>
  ...
  // The full 18-hole scorecard toggle and grid, shared between the
  // Round Complete celebration screen and the live scoring screen - both
  // need the exact same rendering, so this is defined once here rather
  // than duplicated. Self-contained: computes everything it needs
  // directly from round (component-level state), so it can be called
  // from anywhere without needing anything passed in.
  function FullScorecardGrid() {
    const g = GAMES[round.game];
  ...
  if (screen === "card" && round && computed) {
    ...
    const matchPlayResult = MATCH_PLAY_GAMES.includes(round.game) ? (() => {
      ...
      const truncatedRound = { ...round, scores: truncatedScores };
      const truncatedComputed = computeRoundScoring(truncatedRound);
      return computeMatchPlayResult(truncatedRound, truncatedComputed);
    })() : null;
          ...
          {g.oneTeamScore && round.tournamentId ? (
            ...
            <div className="gsc-card gsc-winner-card" style={{ textAlign: "center" }}>
              ...
              {matchPlayResult.isDecided && (
                <div style={{ marginTop: 6 }}>
                  <span className="gsc-chip gsc-lead">MATCH DECIDED</span>
                </div>
              )}
              <div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 14 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, color: "#8a8a80" }}>Halved</div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: "#8a8a80" }}>{matchPlayResult.holesHalved}</div>
              ...
              <div style={{ fontSize: 11, color: "#8a8a80", marginTop: 10 }}>Thru {matchPlayResult.holesPlayed} holes</div>
            </div>
          ) : (
          <div className="gsc-card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
